...
export class DshContextShadow {
 ...
 constructor(service=new ContextService(),private readonly attachments:DshImageAttachmentGateway=new LocalDshImageAttachments()){this.client=new ContextProtocolClient(request=>service.handle(request));const initialized=this.client.call("initialize");if(!initialized.ok)throw new Error(initialized.error.message);}
 async prepare(context:Context,_model:Model<Api>,key:BranchKey={sessionId:"provider-call",branchId:"root"}):Promise<Context>{const encoded=JSON.stringify([key.sessionId,key.branchId]);try{const canonical=await Promise.all(context.messages.map((message,index)=>primeToDshAsync(primeMessage(message),{admitImages:(images)=>this.attachments.admitPrimeImages(images)},`prime-${createHash("sha256").update(`${index}:`).update(stableJson(message)).digest("hex").slice(0,32)}`)));const response=this.client.call("session/sync-canonical",{key,messages:canonical,expectedRevision:this.revisions.get(encoded)??0});if(!response.ok){this.stats.errors++;return Promise.resolve(context);}const projection=this.client.call("project",{key});if(!projection.ok){this.stats.errors++;return Promise.resolve(context);}const roundTrip=await Promise.all(projection.result.messages.map(message=>dshToPrimeAsync(message,{resolveImage:(attachment)=>this.attachments.resolveDshImage(attachment)})));if(stableJson(roundTrip)!==stableJson(context.messages)){this.stats.skips++;return Promise.resolve(context);}this.revisions.set(encoded,response.result.revision);this.stats.syncs++;this.stats.lastMessageCount=response.result.messageCount;}catch{this.stats.skips++;}return Promise.resolve(context);}
}
