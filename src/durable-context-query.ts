...
const OBJECT_VERSION = "prime-agent-dsh/derived-object-v1";
...
export interface ProvenanceTrace {
  ...
  /** False only for the bounded compatibility rendering of a source entry. */
...
interface Loaded { head: string; commitDigest: string; commit: RecordValue; object: RecordValue; branchId: string }
...
export class DurableContextQuery {
  ...
  private load(cutoff = Number.MAX_SAFE_INTEGER): { values: Loaded[]; corrupt: number } {
    ...
    for (const head of names) try {
      ...
      if (!object || object.version !== OBJECT_VERSION || object.bindingDigest !== this.bindingDigest || object.sourceDigest !== commit.sourceDigest || object.effectiveDigest !== commit.effectiveDigest || !Array.isArray(object.sourceEntryDigests) || !(object.sourceEntryDigests as unknown[]).every(v => typeof v === "string" && SHA256.test(v)) || !Array.isArray(object.effective) || digest(object.effective) !== object.effectiveDigest || !Array.isArray(object.effectiveEntryDigests) || object.effectiveEntryDigests.length !== object.effective.length || object.effective.some((v, i) => digest(v) !== (object.effectiveEntryDigests as unknown[])[i]) || !compatibility || compatibility.version !== STORE_VERSION || compatibility.sessionId !== this.sessionId || compatibility.revision !== commit.generation || compatibility.messageCount !== object.effective.length || !Array.isArray(compatibility.messages) || digest(compatibility.messages) !== object.effectiveDigest || compatibility.sourceDigest !== commit.sourceDigest || compatibility.effectiveDigest !== commit.effectiveDigest || typeof compatibility.branchId !== "string" || !Array.isArray(compatibility.entries) || typeof compatibility.cropped !== "boolean") throw new Error();
      values.push({ head, commitDigest, commit, object, branchId: compatibility.branchId });
  ...
  query(request: ContextQueryRequest): ContextQueryPage {
    ...
    for (const v of selected) {
      if (scope === "effective") (v.object.effective as Json[]).forEach((value, index) => docs.push({ loaded: v, index, text: semanticText(value), value, digest: (v.object.effectiveEntryDigests as string[])[index], truncated: false, exact: true }));
