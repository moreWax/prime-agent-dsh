    ...
    for (const head of names) try {
      ...
      if (object.version === LEGACY_OBJECT_VERSION) {
        ...
        const readBody = (bodyDigest: unknown): Json => {
          ...
          const raw = safeText(join(this.root, "bodies", `${bodyDigest}.json`)).trim();
          if (hashText(raw) !== bodyDigest) throw new Error();
          const value = JSON.parse(raw) as Json; if (digest(value) !== bodyDigest) throw new Error(); return value;
        };
